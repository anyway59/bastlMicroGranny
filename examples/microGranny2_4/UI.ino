...
const uint16_t granSizeMap[16] PROGMEM ={
...
const uint16_t shiftSpeedMap[20] PROGMEM ={
...
uint16_t curveMap(uint8_t value, uint8_t numberOfPoints, uint16_t * tableMap){
  //  value=value>>2;
  ...
  //  return (myMap(value-inMin,inMax-inMin, outMax-outMin) +outMin);//
...
void dimLeds(){
  for(uint8_t i=0;i<NUMBER_OF_SOUNDS;i++) hw.setLed(bigButton[i],false);
...
void setEnd(unsigned char _sound){
  ...
  if(revMidi){
    endIndex=startIndex+1, startIndex=endIndex-getVar(_sound,END);
...
void renderTweaking(unsigned char _page){
  ...
  if(wave.isPlaying()){
    ...
    case 1:
      // if(!hw.knobFreezed(0)){
      ll=getVar(_sound,LOOP_LENGTH);
      if(sync) loopLength=pgm_read_word_near(usefulLengths+(ll>>3)); //novinka
      else loopLength=curveMap(ll<<1,GRAIN_MAP_POINTS, granSizeMap);//ll<<LOOP_LENGTH_SHIFT;
      //}
      if(!hw.knobFreezed(1)) {
        //if(sync) shiftSpeed=((long)getVar(_sound,SHIFT_SPEED)-128)<<SHIFT_SPEED_SYNC_SHIFT;
        //else 
        shiftSpeed=(long)curveMap(getVar(_sound,SHIFT_SPEED),SHIFT_SPEED_POINTS,shiftSpeedMap)-16000;//((long)getVar(_sound,SHIFT_SPEED)-128)<<SHIFT_SPEED_SHIFT;
      }
...
void renderCombo(){
  ...
  if(shift && !hw.buttonState(PAGE)){
    ...
    if(hw.justPressed(REC)){
      if(instantLoop==2){
        if(!sync){
          combo=true;
          setVar(activeSound,START,instantStart/startGranule);
          setVar(activeSound,END,instantEnd/startGranule);
          instantLoop=0;
          hw.freezeAllKnobs();
          loadValuesFromMemmory(activeSound);
        /*
        startIndex=getVar(activeSound,START);
         startPosition=startIndex*startGranule;
         setEnd(activeSound);
         */
  ...
  if(combo){
    ...
    for(uint8_t i=0;i<NUMBER_OF_BUTTONS;i++)  _count+=hw.buttonState(i);
...
void renderKnobs(){
  ...
  for(uint8_t i=0;i<NUMBER_OF_KNOBS;i++){
    ...
    if(hw.knobFreezed(i)) {
      ...
      if(inBetween( scale(_knobValue,KNOB_BITS,variableDepth[_variable]), scale(_lastKnobValue,KNOB_BITS,variableDepth[_variable]),was ) || (abs(_knobValue-_lastKnobValue)>15) ) hw.unfreezeKnob(i);//,showForWhile(knobLabel(page,i)),lastMoved==i; //external unfreez
      ...
      if(lastMoved==i){
        if(_variable==RATE){
          if(tuned){
            _value=pitch-36;
            //  _value=(_value,1024,43);
            // _value-=36;
          }
          else _value=pitch-360;//_value-=860;

        }
        else if(_variable==LOOP_LENGTH ){
          if(sync){
            ...
            if(_value<5) _value=0;
            else if(_value<10) _value=1;
            else  _value-=9;//map16(_value,10,128,1,128);
          }

        }
        else if(_variable==SHIFT_SPEED){
          if(_value<110) _value-=110; // _value=map16(_value,0,110,-128,-1);
          else if(_value>146) _value-=146;//  _value=map16(_value,135,256,1,128);
          else _value=0;
...
void renderDisplay(){
  ...
  if(instantLoop==2){
    ...
    for(uint8_t i=0;i<4;i++) hw.lightNumber(LINES,i);
    noDots();
  } 
  else{
    showSampleName(); 
