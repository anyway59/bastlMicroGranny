...
//long legatoPosition; 
//const uint16_t
const uint16_t noteSampleRateTable[49] PROGMEM ={/*0-C*/
...
//  sideNote=EEPROM.read(SIDE_NOTE);
  ...
  //if(inputChannel>6) velo=true;
  //else velo=false;
  //inputChannel=inputChannel%6;
 ...
 if(_number>6) _number--;
  if(_channel==inputChannel){
    if(_value==0) putNoteOut(_number);
      ...
      midiVelocity=126;
     // if(velo) midiVelocity=_value;
    //  else  midiVelocity=126;
//      if(inputChannel>6) midiVelocity=_value;
  //    else midiVelocity=126;  
      putNoteIn(_number);
    }
